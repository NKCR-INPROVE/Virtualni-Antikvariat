        ...
        <xsl:variable name="request" select="//oai:OAI-PMH/oai:request" />
            ...
            <xsl:for-each select="//marc:record" >
                ...
                <doc>
                    ...
                    <field name="ccnb" update="removeregex"><xsl:value-of select="marc:datafield[@tag='015']/marc:subfield[@code='a']"/></field>
                    <field name="isbn" update="removeregex"><xsl:value-of select="marc:datafield[@tag='020']/marc:subfield[@code='a']"/></field>
                    <field name="issn" update="removeregex"><xsl:value-of select="marc:datafield[@tag='022']/marc:subfield[@code='a']"/></field>
                    <field name="pocet_doc" update="inc">1</field>
                    
                    
                    <xsl:call-template name="pocet_ex" />
                    <field name="oai" update="removeregex"><xsl:value-of select="$request"/></field>
			...
			<xsl:if test="(nkf:hasUKF(marc:datafield[@tag=996]/marc:subfield[@code='c']/text()))">
		      ...
		      <xsl:otherwise>
			<field name="zdroj" update="removeregex"><xsl:value-of select="$zdroj"/></field>
		      </xsl:otherwise>
                    </xsl:choose>
                    
                    <field name="titlemd5" update="removeregex"><xsl:value-of select="$title"/></field>
                    <field name="authormd5" update="removeregex"><xsl:value-of select="$autor"/></field>
                    <field name="mistovydani" update="removeregex"><xsl:value-of select="$mistovydani"/></field>
                    <field name="vydavatel" update="removeregex"><xsl:value-of select="$vydavatel"/></field>
                    <field name="datumvydani" update="removeregex"><xsl:value-of select="$datumvydani"/></field>
                    ...
                    <field name="format" update="removeregex"><xsl:value-of select="./marc:controlfield[@tag=990]"/></field>
                    <xsl:variable name="leader"><xsl:value-of select="./marc:leader"/></xsl:variable>
                    <field name="leader" update="removeregex"><xsl:value-of select="$leader"/></field>
                    <field name="ldr19" update="removeregex"><xsl:value-of select="substring($leader, 19,1)"/></field>
                    <field name="leader_format" update="removeregex"><xsl:choose>
		      ...
		      <xsl:otherwise><xsl:value-of select="substring($leader, 8, 1)"/></xsl:otherwise>
                    ...
                    <xsl:for-each select="marc:datafield[@tag=996]/marc:subfield[@code='b']">
                        <field name="carkod" update="removeregex" ><xsl:value-of select="."/></field>
                    </xsl:for-each>
                    <xsl:for-each select="marc:datafield[@tag=996]/marc:subfield[@code='c']">
                        <field name="signatura" update="removeregex" ><xsl:value-of select="."/></field>
                    </xsl:for-each>
                    ...
                    -->
                    <field name="title_suggest" update="removeregex"><xsl:value-of select="exts:prepareCzechLower($xslfunctions, $title)"/>##<xsl:value-of select="$title"/></field>
                    <xsl:for-each select="marc:datafield[@tag=246]">
                        <field name="title2" update="removeregex"><xsl:value-of select="./marc:subfield[@code='a']"/></field>
                    </xsl:for-each>
                    <xsl:if test="marc:datafield[@tag=240]/marc:subfield[@code='a']">
                        <field name="title2" update="removeregex"><xsl:value-of select="marc:datafield[@tag=240]/marc:subfield[@code='a']"/></field>
                    </xsl:if>
                    <xsl:if test="marc:datafield[@tag=130]/marc:subfield[@code='a']">
                        <field name="title2" update="removeregex"><xsl:value-of select="marc:datafield[@tag=130]/marc:subfield[@code='a']"/></field>
                    </xsl:if>
					<!--
                    <xsl:for-each select="marc:datafield[@tag=700]">
                        <field name="author" update="removeregex">
                            <xsl:value-of select="./marc:subfield[@code='a']"/>
                        </field>
                    </xsl:for-each>
                    <xsl:if test="marc:datafield[@tag=100]/marc:subfield[@code='a']">
                        <field name="author" update="removeregex"><xsl:value-of select="marc:datafield[@tag=100]/marc:subfield[@code='a']"/></field>
                    </xsl:if>
