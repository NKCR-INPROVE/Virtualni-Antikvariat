<xsl:stylesheet xmlns:marc="http://www.loc.gov/MARC21/slim" 
    ...
    <xsl:for-each select="//marc:record" >
      ...
      <doc>
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
            <field name="zdroj" update="add">
              <xsl:value-of select="$zdroj"/>
            </field>
          </xsl:otherwise>
        </xsl:choose>
                    
        <field name="titlemd5" update="add">
          <xsl:value-of select="$title"/>
        </field>
        <field name="authormd5" update="add">
          <xsl:value-of select="$autor"/>
        </field>
        <field name="mistovydani" update="add">
          <xsl:value-of select="$mistovydani"/>
        </field>
        <field name="vydavatel" update="add">
          <xsl:value-of select="$vydavatel"/>
        </field>
        <field name="datumvydani" update="add">
          <xsl:value-of select="$datumvydani"/>
        </field>
        ...
        -->
        <field name="title_suggest" update="add">
          <xsl:value-of select="exts:prepareCzechLower($xslfunctions, $title)"/>##<xsl:value-of select="$title"/>
        </field>
        <xsl:for-each select="marc:datafield[@tag=246]">
          <field name="title2" update="add">
        ...
        <xsl:if test="$author != ''">
          <field name="author" update="add">[<xsl:value-of select="$author" />]</field>
        </xsl:if>
  ...
  <xsl:template name="export">
    <xsl:variable name="f100a">
      <xsl:call-template name="autor_export" />
    </xsl:variable>
    <xsl:variable name="csv">
      <xsl:value-of select="slouceni:toCSV(
...
string(concat(normalize-space(marc:datafield[@tag='260']/marc:subfield[@code='a']), 
  ...
  normalize-space(marc:datafield[@tag='260']/marc:subfield[@code='c']))))" />
    </xsl:variable>
    <field name="export_json">
      <xsl:value-of select="slouceni:fromCSV($csv)" />
    </field>
    <field name="export">
  ...
  <xsl:template name="autor_export">
    <xsl:choose>
      <xsl:when test="marc:datafield[@tag='100']/marc:subfield[@code='a']">
        ...
        <xsl:variable name="f100ind">
          <xsl:value-of select="marc:datafield[@tag='100']/@ind1" />
        </xsl:variable>
        <xsl:choose>
          <xsl:when test="marc:datafield[@tag='100']/@ind1 = '1'">
            <xsl:value-of select="concat(substring-after($f100a, ','), ', ', substring-before($f100a, ','))" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="$f100a" />
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:otherwise>
