 ...
 * @author alberto
 */
@WebServlet(value = "/original") 
public class OriginalServlet extends HttpServlet {

  public static final Logger LOGGER = Logger.getLogger(OriginalServlet.class.getName());
...
//                }
        ...
        InputStream stylesheet = Options.class.getResourceAsStream("original.xsl");
  ...
  private void fromDb(String id, PrintWriter out, boolean html) throws SQLException, TransformerException {
    if (html) {
      ...
      InputStream stylesheet = Options.class.getResourceAsStream("marc.xsl");
      StreamSource xslt = new StreamSource(stylesheet);
      transformer = tfactory.newTransformer(xslt);
      StreamResult destStream = new StreamResult(new StringWriter());
      transformer.transform(new StreamSource(new StringReader(IndexerQuery.getOriginalXml(id))), destStream);
      StringWriter sw = (StringWriter) destStream.getWriter();
      out.print(sw.toString());
    } else {
      String xml = IndexerQuery.getOriginalXml(id);
